...
if is_lora:
    ...
    model.eval()
states_value = None
gen_args = PIPELINE_ARGS(temperature = 0.9, top_p = 0.96, top_k = 20, # top_k = 0 then ignore
                        alpha_frequency = 0.25,
                        alpha_presence = 0.25,
                        alpha_decay = 0.996, # gradually decay the penalty
                        token_ban = [], # ban the generation of some tokens
                        token_stop = [0,1,3319], # stop generation whenever you see any token here
                        chunk_len = 512)
cat_char = '🐱'
bot_char = '🤖'
instruction ='根据给定的短文，执行以下任务：列举出来Lawrence Wong Shyun Tsai曾经担任过的职务。'
